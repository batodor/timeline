...
const Users: User[] = array(USERS_COUNT).map((i) => ({
	id: i,
...
function generateFile(time): File {
	return {
		...
		projectName: faker.random.arrayElement(Projects).title,
...
function generateComment(time): Comment {
	...
	return {
		...
		userId: faker.random.arrayElement(Users).id,
		fileId: file.id,
		fileName: faker.internet.domainName() + faker.random.arrayElement(FILE_TYPES),
...
const timelineFabric = (f?: File | Project | User) => {
	return {
		...
		subscribe(cb: (v: File | Comment) => void) {
			...
			return Rx.Observable.merge(files$.filter(filter), comments$.filter(filter)).subscribe(cb);
		}
	};
};
